...
def richer_than_england
  ...
  FROM
    countries
  WHERE
    (gdp / population) > (SELECT (gdp / population) FROM countries 
...
def neighbors_of_certain_b_countries
  ...
  FROM
    countries
  WHERE
    continent IN (SELECT continent FROM countries 
...
def population_constraint
  ...
  FROM
    countries
  WHERE
    population BETWEEN (select population from countries where name = 'Canada')+1
